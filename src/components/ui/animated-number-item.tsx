...
const AnimatedNumberItem: React.FC<AnimatedNumberItemProps> = ({ number, size, duration = 200, order = 'asc' }) => {
    ...
    <div className="AnimatedNumberItem" style={{ height: size }}>
      <div
        ...
        {Array.from({ length: 10 }).map((_, number) => (
          <div
            key={number}
            className="AnimatedNumberItem__number main-text-primary font-semibold"
            style={{
              fontSize: size,
              height: size,
            }}>
